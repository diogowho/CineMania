...
int addMovieInteractive(MovieDatabase* db) {
    ...
    if (fgets(buffer, MAX_DESCRIPTION_LENGTH, stdin) != NULL) {
        ...
        while (token != NULL && newMovie.actorCount < MAX_ACTORS_PER_MOVIE) {
            trimString(token);
            if (strlen(token) > 0) {
                strncpy(newMovie.actors[newMovie.actorCount], token, MAX_ACTOR_NAME_LENGTH - 1);
...
int editMovie(MovieDatabase* db, int code) {
    ...
    /* Note: According to spec, can edit: title, genres, year, duration, rating, favorite, revenue */
        ...
        case 2: /* Genres */
            ...
            if (fgets(genreInput, MAX_STRING_LENGTH, stdin) != NULL) {
                ...
                if (movie->genreCount > 0) {
                    ...
                    printf("Error: At least one valid genre is required. Changes not saved.\n");
                    return 0;
                }
            }
            break;
...
int searchByTitle(const MovieDatabase* db, const char* searchTerm, int* results, int maxResults) {
    int i, count = 0;
...
int searchByGenre(const MovieDatabase* db, Genre genre, int* results, int maxResults) {
    int i, j, count = 0;
...
int searchByDirector(const MovieDatabase* db, const char* director, int* results, int maxResults) {
    int i, count = 0;
...
int searchByActor(const MovieDatabase* db, const char* actor, int* results, int maxResults) {
    int i, j, count = 0;
