...
void printTableHeader(void) {
    ...
    printf("%-6s | %-40s | %-25s | %-25s | %-6s | %-4s | %-6s | %-8s | %-8s\n",
           "Code", "Title", "Genres", "Director", "Year", "Dur", "Rating", "Favorite", "Revenue");
    printLine(150);
}
...
void printMovieRow(const Movie* movie) {
    ...
    for (i = 0; i < movie->genreCount && i < 3; i++) {
        ...
        strncat(genresStr, getGenreName(movie->genres[i]), MAX_STRING_LENGTH - strlen(genresStr) - 1);
    }
    ...
    if (strlen(movie->title) > 40) {
        printf("%-6d | %.37s... | %-25s | %-25s | %-6d | %-4d | %6.1f | %8d | %8.2f\n",
               movie->code,
               movie->title,
               genresStr,
               movie->director,
...
void displayMoviesTable(const MovieDatabase* db, const int* indices, int count, int usePagination) {
    ...
    if (usePagination) {
        ...
        while (page * LINES_PER_PAGE < count) {
            ...
            for (i = page * LINES_PER_PAGE; i < (page + 1) * LINES_PER_PAGE && i < count; i++) {
                if (indices != NULL) {
